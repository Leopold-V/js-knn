...
const DistanceFromTargetArray = async (sepal_length, sepal_width, petal_length, petal_width) => {
    const arr = await getdata();
    const copyBeforeModif = JSON.parse(JSON.stringify(arr));
    const long = Math.sqrt(sepal_length*sepal_length + sepal_width*sepal_width + petal_length*petal_length + petal_width*petal_width);
...
const FindSpecies = async (sepal_length, sepal_width, petal_length, petal_width) => {
    const arr = await DistanceFromTargetArray(sepal_length, sepal_width, petal_length, petal_width);
    const count = {};
